...
success_orders AS (
    SELECT 
        user_id,
        avg(order_price) as orders_sum_for_user
    FROM orders_with_price
    LEFT JOIN user_actions ON orders_with_price.order_id = user_actions.order_id
    WHERE date_part('year', time) = 2022
        AND date_part('month', time) = 9
        and date_part('day', time) BETWEEN 1 AND 7
    GROUP BY user_id
),
ads_sum as (
    SELECT
...
SELECT 
    ads_campaign,
    round(avg(orders_sum_for_user), 2) as avg_check
FROM ads_sum
GROUP BY ads_campaign
HAVING ads_campaign != 'other'
